...
export const REVIEWER_SUBAGENT_DEFINITION = {
    id: "reviewer",
    name: "Code Reviewer",
    role: "reviewer",
    allowedTools: [
    ...
    description: "Reviews diffs, checks architecture compliance, identifies security risks, and finds test gaps.",
};
// ─── Review Patterns ────────────────────────────────────────────────────────
/**
...
export class ReviewerSubAgent extends BaseSubAgent {
    constructor(toolExecutor, eventEmitter, approvalHandler) {
        super(REVIEWER_SUBAGENT_DEFINITION, toolExecutor, eventEmitter, approvalHandler);
    }
    async execute(task, tools) {
        const startTime = Date.now();
        const reviewType = task.context.metadata?.reviewType;
        switch (reviewType) {
            case "security":
     ...
     */
    async handleSecurityReview(task, tools, startTime) {
        const findings = [];
        ...
        return this.buildReviewResult(task.id, "security", findings, startTime, tools);
    }
    /**
     * Perform an architecture compliance review.
     */
    async handleArchitectureReview(task, tools, startTime) {
        const findings = [];
        const filePaths = task.context.filePaths ?? [];
        for (const filePath of filePaths) {
            try {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        if (pattern.test(lines[i])) {
                            findings.push({
                                ...
                                filePath,
                                line: i + 1,
                                suggestion: "Consider refactoring this to follow project conventions.",
                            });
                        }
                    }
                }
        ...
        return this.buildReviewResult(task.id, "architecture", findings, startTime, tools);
    }
    /**
     * Review a git diff for issues.
     */
    async handleDiffReview(task, tools, startTime) {
        try {
            ...
            return this.buildReviewResult(task.id, "diff", findings, startTime, tools, result.output);
        }
        catch (error) {
            return {
                taskId: task.id,
     ...
     */
    async handleFullReview(task, tools, startTime) {
        const findings = [];
        ...
        for (let i = 0; i < lines.length; i++) {
            ...
            if (addedLine.length > 500) {
                findings.push({
                    category: "quality",
                    severity: "low",
                    description: "Very long line added — consider breaking into multiple lines",
                    line: i + 1,
                });
            }
        }
     ...
     */
    buildReviewResult(taskId, reviewType, findings, startTime, tools, diffContent) {
...
//# sourceMappingURL=reviewer-subagent.js.map